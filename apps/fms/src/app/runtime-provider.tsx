...
import { createFmsApiClient } from "../shared/api/client";
...
const mapErrorToMessage = (error: unknown): string => {
  ...
  return "Could not initialize FMS session";
};

interface FmsRuntimeContextValue {
...
const FmsRuntimeContext = createContext<FmsRuntimeContextValue | null>(null);
...
export function FmsRuntimeProvider({ children }: PropsWithChildren) {
  ...
  const [apiClient, setApiClient] = useState<ReturnType<typeof createFmsApiClient>>(anonymousClient);
  ...
  const applyAuthenticatedState = useCallback(
    (nextSession: AuthSession, token: string) => {
      ...
      const authedClient = createFmsApiClient(nextSession, {
        apiOrigin,
        authToken: token,
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        if (stored) {
          try {
            const restoredClient = createFmsApiClient(stored.session, {
              apiOrigin,
        ...
        const developmentClient = createFmsApiClient(undefined, { apiOrigin });
        ...
        const authenticatedClient = createFmsApiClient(loginResponse.session, {
          apiOrigin,
  ...
  const value = useMemo<FmsRuntimeContextValue>(
    () => ({
      session,
      status,
      isBootstrapping,
  ...
  return <FmsRuntimeContext.Provider value={value}>{children}</FmsRuntimeContext.Provider>;
}

export const useFmsRuntime = (): FmsRuntimeContextValue => {
  const context = useContext(FmsRuntimeContext);
  if (!context) {
    throw new Error("useFmsRuntime must be used inside FmsRuntimeProvider");
  }
