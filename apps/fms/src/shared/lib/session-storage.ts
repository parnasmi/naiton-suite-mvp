...
const FMS_AUTH_STORAGE_KEY = "naiton.fms.auth";
const FALLBACK_STORAGE_KEYS = [FMS_AUTH_STORAGE_KEY, "naiton.shell.auth"];
...
export const writeStoredAuthState = (state: StoredAuthState): void => {
  ...
  window.localStorage.setItem(FMS_AUTH_STORAGE_KEY, JSON.stringify(state));
};
...
export const clearStoredAuthState = (): void => {
  ...
  window.localStorage.removeItem(FMS_AUTH_STORAGE_KEY);
};
