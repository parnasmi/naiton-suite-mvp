...
const mapErrorToMessage = (error: unknown): string => {
  ...
  return "Could not initialize CRM session";
};

interface CrmRuntimeContextValue {
...
const CrmRuntimeContext = createContext<CrmRuntimeContextValue | null>(null);
...
export function CrmRuntimeProvider({ children }: PropsWithChildren) {
  ...
  const [apiClient, setApiClient] = useState<ReturnType<typeof createCrmApiClient>>(anonymousClient);
  ...
  const applyAuthenticatedState = useCallback(
    (nextSession: AuthSession, token: string) => {
      ...
      const authedClient = createCrmApiClient(nextSession, {
        apiOrigin,
        authToken: token,
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        if (stored) {
          try {
            const restoredClient = createCrmApiClient(stored.session, {
              apiOrigin,
        ...
        const developmentClient = createCrmApiClient(undefined, { apiOrigin });
        ...
        const authenticatedClient = createCrmApiClient(loginResponse.session, {
          apiOrigin,
  ...
  const value = useMemo<CrmRuntimeContextValue>(
    () => ({
      session,
      status,
      isBootstrapping,
  ...
  return <CrmRuntimeContext.Provider value={value}>{children}</CrmRuntimeContext.Provider>;
}

export const useCrmRuntime = (): CrmRuntimeContextValue => {
  const context = useContext(CrmRuntimeContext);
  if (!context) {
    throw new Error("useCrmRuntime must be used inside CrmRuntimeProvider");
  }
