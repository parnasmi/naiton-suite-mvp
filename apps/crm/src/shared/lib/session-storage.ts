...
const CRM_AUTH_STORAGE_KEY = "naiton.crm.auth";
const FALLBACK_STORAGE_KEYS = [CRM_AUTH_STORAGE_KEY, "naiton.shell.auth"];
...
export const writeStoredAuthState = (state: StoredAuthState): void => {
  ...
  window.localStorage.setItem(CRM_AUTH_STORAGE_KEY, JSON.stringify(state));
};
